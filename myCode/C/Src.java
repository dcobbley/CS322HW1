...
class Plus extends IExpr {
  ...
  Code compileTo(Reg reg, Code next) {
    Reg tmp = new Reg();
    return l.compileTo(tmp, r.compileTo(reg, new Op(reg, tmp, '+', reg, next)));
  }
}
...
class Mult extends IExpr {
	  ...
	  Code compileTo(Reg reg, Code next) {
		  Reg tmp = new Reg();
		  return l.compileTo(tmp,
		  r.compileTo(reg, new Op(reg, tmp, '*', reg, next)));
	  }
}
class Halve extends IExpr {
	  private IExpr arg;
	    Halve(IExpr arg) { this.arg = arg; }

	  int eval(Memory mem) { 
		  return arg.eval(mem)/2; 
	  }
	  String show() { 
		  return "(" + arg.show() + " /2 "  + ")"; 
	  }
	  ...
	  Code compileTo(Reg reg, Code next) {
		  Reg tmp = new Reg();
    	  return arg.compileTo(reg, new HalveOp(reg, next));
	  }
}
...
class Not extends BExpr {
  private BExpr arg;
  Not( BExpr arg) { this.arg = arg; }

  boolean eval(Memory mem) { return !arg.eval(mem); }
  String show()  { return "(" + " ! " + arg.show() + ")"; }

  Code compileTo(Reg reg, Code next) {
    Reg tmp = new Reg();
    return arg.compileTo(reg, new NotOp(reg, next));
  }
}

class Even extends BExpr {
  private IExpr arg;
  Even(IExpr arg) { this.arg = arg; }

  boolean eval(Memory mem) { 
	  return (arg.eval(mem) & 1)==0; }
  String show()  { return "( Even " + arg.show()  + ")"; }

  Code compileTo(Reg reg, Code next) {
    Reg tmp = new Reg();
    return arg.compileTo(reg, new EvenOp(reg, next));
  }
}
